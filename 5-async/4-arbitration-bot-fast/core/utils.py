...
async def dump_json_to_file(json_data, filename):
    try:
        ...
        raise StoragePermissionError(f"Нет прав для записи в файл {filename}") from error
    except OSError as error:
        if error.errno == 28:
            raise DiskFullError(f"Недостаточно места на диске для сохранения {filename}")
        raise LocalStorageError(f"Ошибка при записи файла {filename}: {error}") from error
    except Exception as error:
        raise LocalStorageError(f"Неизвестная ошибка при сохранении файла {filename}") from error
